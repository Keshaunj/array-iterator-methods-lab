...
/* callbackfn: (value: number , index: number, array: number[]) => value
is S
, thisArg?: any): S[]
A function that accepts up to three arguments. The filter method calls the callbackfn function one time for each*/

/* RANGES SYNATX ARE X >= && <= X */
// Complete the exercise in the space below:

veryOldInventors = inventors.filter(function (investor) {
...
// Complete the exercise in the space below:
/*inventorsNames = inventors.map(function(inventors){
return inventorNames.names
})*/
...
// Complete the exercise in the space below:
sortedByBirthYear = inventors.sort((x, y) => x.year - y.year);

// using w3 schools and going at first i had ortedByBirthYear = inventors.sort ((1473,1980)=> x.year - y.year); etc
...
//isAdultPresent = devs.some(fuction(personAge))
/*devs.forEach( k = 0 , k < devs.length, k++)
 const  perosnAge = devs[k];
   const  yearsOld = 2024 - devs.year;
   if (personAge >= 18){
    return isAdultPresent = true;
   }*/
   isAdultPresent = devs.some(function(age) {
const personAge = 2024 - age.year;
return personAge>= 18;
   })
...
// Complete the exercise in the space below:
 isEveryone19OrOlder = devs.every(function(old){
    const peopleOld = 2024 - old.year;
    return peopleOld >= 19;
 }
)
// it return falesy because the peron wes was just born in 2015
...
/*idx = comments.findIndex(function(idx){
}
)*/
idx = comments.findIndex (gotIt => gotIt.id ===  123523 )
// Check your work:
